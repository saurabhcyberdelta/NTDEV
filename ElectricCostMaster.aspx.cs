...
public partial class ElectricCostMaster : basepage
{
    ...
    void bndGridCost()
    {
        gvCost.DataSource = tms.GetElectricCost(Convert.ToInt32(ddlSelectFacility.SelectedValue));
        gvCost.DataBind();
    ...
    protected void dvCost_ItemInserting(object sender, DetailsViewInsertEventArgs e)
    {
        ...
        DropDownList ddlVendor = (DropDownList)dvCost.FindControl("ddlVendor");
        String Result = tms.AddNewElectricCabCost(Convert.ToDateTime(effectiveDate.Text), Convert.ToInt32(ddlveh.SelectedValue),
            Convert.ToDouble(NewRate.Text), Convert.ToInt32(ddlVendor.SelectedValue), 
            MyApplicationSession._UserID, Convert.ToDouble(NewacRate.Text), 
            Convert.ToDouble(FuelRate.Text), Convert.ToInt32(ddlSelectFacility.SelectedValue)).ElementAtOrDefault(0).Result.ToString();
        if (Result == "1")
        {
    ...
    protected void dvAddnewvendor_ItemInserting(object sender, DetailsViewInsertEventArgs e)
    {
        ...
        String Result = tms.AddNewVendorElectricCabCost(Convert.ToDateTime(effectiveDate.Text), Convert.ToInt32(ddlveh.SelectedValue), Convert.ToDouble(NewRate.Text), Convert.ToInt32(ddlVendor.SelectedValue), MyApplicationSession._UserID, Convert.ToDouble(NewACRate.Text), Convert.ToDouble(FuelRate.Text)).ElementAtOrDefault(0).Result.ToString();
        if (Result == "1")
        {
            lblErrorMsg.Visible = false;
            mvCost.ActiveViewIndex = 0;
    ...
    {
        int id = Convert.ToInt32(gvCost.DataKeys[e.RowIndex].Value);
        tms.DeleteElectricCabCost(id);
        bndGridCost();
    }
}