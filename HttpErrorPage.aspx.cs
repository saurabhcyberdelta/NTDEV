...
public partial class HttpErrorPage : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            ex = new HttpException(httpCode, "Safe message for 4xx HTTP codes.", ex);
        }
        else if (httpCode > 499)
        {
            ex = new HttpException(ex.ErrorCode, "Safe message for 5xx HTTP codes.", ex);
        }
        else
        {
            ex = new HttpException(httpCode, "Safe message for unexpected HTTP codes.", ex);
        }
