...
public partial class EmpReplicateSchedule : basepage2
{
    ...
    protected Boolean checkemp(object strArg)
    {
        int val = Convert.ToInt32(strArg);

        if (val == MyApplicationSession._UserID)
        {
            return true;
        }
        else
    ...
    protected void btnReplicate_Click(object sender, EventArgs e)
    {
        try
        {
            if (Page.IsValid == true)
            {
                ...
                if (result <= 0)
                {
                    lblSaveMsg.Text = "Schedule not replicated successfully.";
                }
                else if (result == 1)
                {
                    lblSaveMsg.Text = "Schedule Replicated successfully.";
                }
                else if (result == 2)
                {
