...
public partial class analysisDetails : basepage
{
    ...
    {
        Grdanalysis.DataSource = tmscontext.getAnalysisDetails(Convert.ToInt32(ddlYear.SelectedValue), Convert.ToInt32(ddlFacility.SelectedValue));
    ...
    protected void lbtnAnalysisName_Click(object sender, EventArgs e)
    {
        ...
        tmscontext.addanalysisdetail(years, facid, Convert.ToInt32(id), txtjan, txtfeb, txtmar, txtapr, txtmay, txtjun, txtjul, txtaug, txtsep, txtoct, txtnov, txtdec);
        String lbtnnavigateurl = new EncryptQueryString().Encrypt("id=" + id + "&facid=" + facid.ToString() + "&years=" + years.ToString());
        string jScript = "window.open('dashboardanalysis.aspx?" + lbtnnavigateurl + "','','');";
