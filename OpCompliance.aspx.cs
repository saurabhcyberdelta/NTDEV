...
public partial class OpCompliance : basepage
{
    ...
    private void BndGvDetail()
    {
        try
        {
            gvDetail.DataSource = tms.GetOPSCheckData(_CabId, Convert.ToDateTime(_SelDate), _facilityID, _vendorID, rdoTripType.SelectedValue.ToString(), lstShift.SelectedItem.ToString());
            gvDetail.DataBind();
            txtRegistrationNo.Visible = true;
            txtRegistrationNo.Text = tms.GetOPSCheckData(_CabId, Convert.ToDateTime(_SelDate), _facilityID, _vendorID, rdoTripType.SelectedValue.ToString(), lstShift.SelectedItem.ToString()).ElementAtOrDefault(0).vehicleRegistrationNo.ToString();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (EditRows == gvDetail.Rows.Count && IsOther == 0)
            {
                ...
                refID = Convert.ToInt32(tms.UpdateOPCheckMain(Convert.ToDateTime(_SelDate), _CabId, "Delete", IsOther, OtherRemark, OtherPenalty, _facilityID, _vendorID, regno, rdoTripType.SelectedValue.ToString(), lstShift.SelectedItem.ToString()).ElementAtOrDefault(0).ID.ToString());
                if (refID == -1)
            ...
            {
                string regno = txtRegistrationNo.Text;
                refID = Convert.ToInt32(tms.UpdateOPCheckMain(Convert.ToDateTime(_SelDate), _CabId, MyApplicationSession._UserID.ToString(), IsOther, OtherRemark, OtherPenalty, _facilityID, _vendorID, regno,rdoTripType.SelectedValue.ToString(),lstShift.SelectedItem.ToString()).ElementAtOrDefault(0).ID.ToString());
                foreach (GridViewRow row in gvDetail.Rows)
                {
                    drdList = (DropDownList)(gvDetail.Rows[row.RowIndex].Cells[1].FindControl("ddlStatus"));
