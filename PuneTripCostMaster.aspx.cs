...
public partial class PuneTripCostMaster : basepage
{
    ...
    void bndGridCost()
    {
        gvCost.DataSource = tms.GetPuneTripCost(Convert.ToInt32(ddlSelectFacility.SelectedValue));
        gvCost.DataBind();
    ...
    protected void dvCost_ItemInserting(object sender, DetailsViewInsertEventArgs e)
    {
        ...
        DropDownList ddlVendor = (DropDownList)dvCost.FindControl("ddlVendor");
        String Result = tms.AddNewPuneCabCost(Convert.ToDateTime(effectiveDate.Text), Convert.ToInt32(ddlveh.SelectedValue), Convert.ToDouble(NewRate.Text), Convert.ToInt32(ddlVendor.SelectedValue), MyApplicationSession._UserID, Convert.ToDouble(NewacRate.Text), Convert.ToDouble(FuelRate.Text)).ElementAtOrDefault(0).Result.ToString();
        if (Result == "1")
        {
    ...
    protected void dvAddnewvendor_ItemInserting(object sender, DetailsViewInsertEventArgs e)
    {
        ...
        String Result = tms.AddNewVendorPuneCabCost(Convert.ToDateTime(effectiveDate.Text), Convert.ToInt32(ddlveh.SelectedValue), Convert.ToDouble(NewRate.Text), Convert.ToInt32(ddlVendor.SelectedValue), MyApplicationSession._UserID, Convert.ToDouble(NewACRate.Text), Convert.ToDouble(FuelRate.Text)).ElementAtOrDefault(0).Result.ToString();
        if (Result == "1")
        {
            lblErrorMsg.Visible = false;
            mvCost.ActiveViewIndex = 0;
